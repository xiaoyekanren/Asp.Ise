...
namespace Zzm.Web
{
    public partial class allinfo : System.Web.UI.Page
    {
        ...
        {
            //这个是手动将各个值打进去- - 。
            if (DropDownList1.SelectedValue.ToString() == "科技南楼B103")
            {
                SqlDataSource1.SelectCommand = "SELECT * FROM Server_detail where PLACE='科技南楼B103' order by IP";
            }
            else
                ...
                if (DropDownList1.SelectedValue.ToString() == "Fit-1-106")
            {
                ...
                if (DropDownList1.SelectedValue.ToString() == "云")
            {
                SqlDataSource1.SelectCommand = "SELECT Server_detail.* FROM Server_detail where PLACE='云' order by IP";
            }
            else
                if (DropDownList1.SelectedValue.ToString() == "All")
            {
