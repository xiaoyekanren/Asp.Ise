...
namespace Zzm.Web
{
    public partial class registinfo : System.Web.UI.Page
    {
        ...
        {
            //这个是手动将各个值打进去- - 。
            if (DropDownList2.SelectedValue.ToString() == "科技南楼B103")
            {
                SqlDataSource2.SelectCommand = "SELECT [PLACE], [IP地址], [USER],  [开始日期], [截止日期], [用途],[备注],ID FROM [use] where [PLACE] = '科技南楼B103' ORDER BY [IP地址]";
            }
            else
                ...
                if (DropDownList2.SelectedValue.ToString() == "Fit-1-106")
            {
                ...
                if (DropDownList2.SelectedValue.ToString() == "云")
            {
                SqlDataSource2.SelectCommand = "SELECT [PLACE], [IP地址], [USER],  [开始日期], [截止日期], [用途],[备注],ID FROM [use] where [PLACE] = '云' ORDER BY [IP地址]";
            }
            else
                ...
                if (DropDownList2.SelectedValue.ToString() == "其他")
            {
                ...
                if (DropDownList2.SelectedValue.ToString() == "天远")
            {
                SqlDataSource2.SelectCommand = "SELECT [PLACE], [IP地址], [USER], [开始日期], [截止日期],[用途],  [备注],ID FROM [use]  where [PLACE] = '天远' ORDER BY [IP地址]";
            }
            else
            {
